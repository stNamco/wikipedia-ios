...
class SamplingController: NSObject {
     ...
     */
    func inSample(stream: EPC.Stream, config: EPC.StreamConfiguration) -> Bool {
     ...
     *   this session
     */
    func cacheSamplingForStream(_ stream: EPC.Stream, inSample: Bool) {
        queue.async {
     ...
     */
    func getSamplingForStream(_ stream: EPC.Stream) -> Bool? {
        queue.sync {
